...
const moduleData = {
  0: {
    ...
    sections: [
      { heading: 'What is Phishing?', content: 'Phishing attacks trick users into clicking malicious links or providing sensitive information. They often impersonate banks, IT departments, or popular services like Google and Microsoft.' },
      { heading: 'Common Signs of Phishing', content: 'Look out for urgent language like "Your account will be closed!", misspelled domain names (e.g. support@g00gle.com), unexpected attachments, and requests for passwords or personal data.' },
      { heading: 'How to Respond', content: 'Never click suspicious links. Verify the sender by contacting the organization directly. Report phishing emails to your IT department immediately.' }
    ],
    tips: [
  ...
  1: {
    ...
    sections: [
      { heading: 'What is Social Engineering?', content: 'Attackers use psychological manipulation to trick employees. Common tactics include pretexting (creating a fake scenario), baiting (offering something enticing), and tailgating (following someone into a restricted area).' },
      { heading: 'Real World Examples', content: 'An attacker calls pretending to be IT support and asks for your password to "fix an issue". Or someone leaves a USB drive labeled "Payroll 2025" hoping an employee will plug it in.' },
      { heading: 'How to Protect Yourself', content: 'Always verify the identity of anyone requesting sensitive information. Follow your organization\'s security policies strictly. Never share passwords, even with IT staff.' }
    ],
    tips: [
  ...
  2: {
    ...
    sections: [
      { heading: 'What Makes a Strong Password?', content: 'A strong password is at least 12 characters long and includes uppercase letters, lowercase letters, numbers, and special characters. Avoid using names, birthdays, or common words.' },
      { heading: 'Password Mistakes to Avoid', content: 'Never reuse passwords across multiple accounts. Avoid simple patterns like "Password123". Do not store passwords in plain text files or sticky notes on your monitor.' },
      { heading: 'Using Password Managers', content: 'Password managers securely store and generate strong unique passwords for every account. This means you only need to remember one master password while staying fully protected.' }
    ],
    tips: [
  ...
  3: {
    ...
    sections: [
      { heading: 'Types of Malware', content: 'Viruses attach to files and spread when opened. Trojans disguise themselves as legitimate software. Spyware secretly monitors your activity. Ransomware encrypts your data and demands a ransom to restore it.' },
      { heading: 'How Malware Spreads', content: 'Malware commonly spreads through email attachments, malicious downloads, infected USB drives, and compromised websites. Even legitimate-looking software can be bundled with malware.' },
      { heading: 'Prevention and Response', content: 'Keep your operating system and software updated. Use reputable antivirus software. Never download software from untrusted sources. If infected, disconnect from the network immediately and contact IT.' }
    ],
    tips: [
...
function ShieldCanvas() {
  ...
  useEffect(() => {
    ...
    const shields = Array.from({ length: 10 }, () => ({ x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, size: 20 + Math.random() * 35, speed: 0.15 + Math.random() * 0.2, alpha: 0.03 + Math.random() * 0.06, pulse: Math.random() * Math.PI * 2, drift: (Math.random() - 0.5) * 0.3 }))
    const particles = Array.from({ length: 40 }, () => ({ x: Math.random() * window.innerWidth, y: Math.random() * window.innerHeight, r: Math.random() * 1.5 + 0.5, dx: (Math.random() - 0.5) * 0.18, dy: -0.1 - Math.random() * 0.2, alpha: Math.random() * 0.25 + 0.05, pulse: Math.random() * Math.PI * 2 }))
    const glows = [{ x: 0, y: 0, r: 280, color: '20,60,180', alpha: 0.07 }, { x: window.innerWidth, y: window.innerHeight, r: 320, color: '60,20,180', alpha: 0.05 }]
    const drawShield = (x, y, size, alpha) => {
      ctx.save(); ctx.translate(x, y); ctx.globalAlpha = alpha
      ctx.strokeStyle = 'rgba(80,160,255,1)'; ctx.lineWidth = 1.2
      ...
      ctx.stroke(); ctx.globalAlpha = alpha * 0.3; ctx.fillStyle = 'rgba(40,100,220,1)'; ctx.fill(); ctx.restore()
    }
    let frame = 0
    const draw = () => {
      frame++; ctx.clearRect(0, 0, canvas.width, canvas.height)
      const bg = ctx.createLinearGradient(0, 0, canvas.width, canvas.height); bg.addColorStop(0, '#030a1a'); bg.addColorStop(0.5, '#050d22'); bg.addColorStop(1, '#030810'); ctx.fillStyle = bg; ctx.fillRect(0, 0, canvas.width, canvas.height)
      ...
      shields.forEach(s => { s.pulse += 0.008; s.y -= s.speed; s.x += s.drift; if (s.y < -s.size * 2) { s.y = canvas.height + s.size; s.x = Math.random() * canvas.width } if (s.x < -s.size) s.x = canvas.width + s.size; if (s.x > canvas.width + s.size) s.x = -s.size; drawShield(s.x, s.y, s.size, s.alpha + 0.02 * Math.sin(s.pulse)) })
      particles.forEach(p => { p.x += p.dx; p.y += p.dy; p.pulse += 0.02; if (p.y < -5) { p.y = canvas.height + 5; p.x = Math.random() * canvas.width } if (p.x < 0) p.x = canvas.width; if (p.x > canvas.width) p.x = 0; ctx.beginPath(); ctx.arc(p.x, p.y, p.r, 0, Math.PI * 2); ctx.fillStyle = `rgba(100,170,255,${p.alpha * (0.6 + 0.4 * Math.sin(p.pulse))})`; ctx.fill() })
